...
exports.postQuotes = function(req, h) {
    ...
    const quotesRequest = req.payload
        //Logger.info(`quotesRequest :[${JSON.stringify(quotesRequest)}]` )
    //Logger.info((new Date().toISOString()), ['IN PAYEEFSP::'], `received: ${metadata}. `)
        //Logger.info('incoming request: ', quotesRequest.quoteId)
        // Logger.info(`incoming request:  [${JSON.stringify(quotesRequest.quoteId)}]`)
        //Saving Incoming request 
    let incomingRequest = {
        headers: req.headers,
        data: req.payload
    ...
    //   transferAmount: {
    //     amount: quotesRequest.amount.amount, //100
    //     currency: quotesRequest.amount.currency //usd
    //   },
    //   payeeFspFee: {
    //     amount: '1',
    //     currency: quotesRequest.amount.currency //usd
